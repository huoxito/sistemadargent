	...
	function _checkValid() {
		if ($this->read('Config')) {
			if ((Configure::read('Session.checkAgent') === false || $this->_userAgent == $this->read('Config.userAgent')) && $this->time <= $this->read('Config.time')) {
                ...
                # linhas alteradas para debugar problema com seções!
                if($this->time > $this->read('Config.time')){
                    $this->__setError(1, 'Sua última interação ocorreu em '.date('d-m-Y H:i:s', $this->time).' e sua seção acabou em '.date('d-m-Y H:i:s', $this->read('Config.time')).'');
                }else{
                    $this->__setError(1, 'Session Highjacking Attempted !!! <hr /> userAgent is '.$this->_userAgent.' userAgent is '.$this->read('Config.userAgent').'');
                }
			}
