...
class AgendamentosController extends AppController {
    ...
    function migraBaixaParaVencimento(){
        ...
        $params = array(
                'conditions' => array('Ganho.status' => '0'),
                'fields' => array('id','datadabaixa','agendamento_id'),
                'order' => 'agendamento_id desc'
            );
        
        # função para migrar os registros de databaixa para datadevencimento
        $this->loadModel('Ganho');
        $item = $this->Ganho->find('all',$params);
        
        foreach($item as $row){
            
            if(!empty($row['Ganho']['datadabaixa'])){
            
                //echo $row['Ganho']['agendamento_id'].'->'.$row['Ganho']['id'].'-->'.$row['Ganho']['datadabaixa'].'<hr />';
                
                $array['Ganho']['datadevencimento'] = $row['Ganho']['datadabaixa'];
                $array['Ganho']['datadabaixa'] = null;
                $this->Ganho->id = $row['Ganho']['id'];
                
                if ( $this->Ganho->save($array,false) ){
                    echo 'Ganho :: '.$row['Ganho']['agendamento_id'].'->'.$row['Ganho']['id'].'--> Campo data de vencimento preenchido para '.$array['Ganho']['datadevencimento'].' e data baixa para '.$array['Ganho']['datadabaixa'].'<hr />';
                }else{
                    print_r($this->Ganho->invalidFields());
                }
            }
        ...
        $params = array(
                'conditions' => array('Gasto.status' => '0'),
                'fields' => array('id','datadabaixa','agendamento_id'),
                'order' => 'agendamento_id desc'
            );
        ...
        $itemG = $this->Gasto->find('all',$params);
        
        foreach($itemG as $row){
            
            if(!empty($row['Gasto']['datadabaixa'])){
            
                //echo $row['Ganho']['agendamento_id'].'->'.$row['Ganho']['id'].'-->'.$row['Ganho']['datadabaixa'].'<hr />';
                $array['Gasto']['datadevencimento'] = $row['Gasto']['datadabaixa'];
                $array['Gasto']['datadabaixa'] = null;
                $this->Gasto->id = $row['Gasto']['id'];
                
                if ( $this->Gasto->save($array,false) ){
                    echo 'Gasto :: '.$row['Gasto']['agendamento_id'].'->'.$row['Gasto']['id'].'--> Campo data de vencimento preenchido para '.$array['Gasto']['datadevencimento'].' e data baixa para '.$array['Gasto']['datadabaixa'].'<hr />';
                }else{
                    print_r($this->Gasto->invalidFields());
                }
            }
        }
